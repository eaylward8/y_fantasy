...
RSpec.describe YFantasy::Transformations::CollectionTransformer do
  describe "#call" do
    ...
    context "with subresources" do
      ...
      it "returns an array of resource instances with subresource instances" do
        results = described_class.new("games").call(data)
    ...
    context "with return_array option set to false" do
      let(:data) do
        {
          games: {
            game: [
      ...
      it "returns a hash with key/value pair of collection name and array of resource instances" do
        results = described_class.new("games", return_array: false).call(data)

        expect(results).to be_a(Hash)
        results[:games].each do |result|
          expect(result.game_weeks).to all(be_a(YFantasy::Game::GameWeek))
        end
      end
    end
